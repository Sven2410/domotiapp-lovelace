...
32. **De recorder van Home Assistant schrijft met VERTRAGING weg, dus twee
   seconden later opvragen is te vroeg.** De cameratijdlijn ververst zichzelf
   zodra een melder aangaat; `history/history_during_period` leest alleen wat er
   al in de database staat. Gemeten op 28 augustus 2026 in de testinstance:
   ...
   Staat de pauze te kort, dan verschijnt het merkje op het beeld WEL meteen
   (dat komt uit `hass`) en blijft de lijst eronder zwijgen -- en dat leest als
   een kapotte lijst in plaats van als een trage database. Vijf seconden is
   genoeg gebleken. Wie iets bouwt dat de recorder uitleest kort nadat er iets
   gebeurd is, hoort deze marge aan te houden.
...
**Tellingen op 28 augustus 2026 (0.29.0):** 866 JS-tests en 526 Python-tests,
alle groen; bundel 604.857 bytes; 158 getekende iconen (het DomotiTech-logo
